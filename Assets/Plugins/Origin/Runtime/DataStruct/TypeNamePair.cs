...
namespace Origin
{
    ...
    public readonly struct TypeNamePair:IEquatable<TypeNamePair>
    {
        ...
        {
            m_Type = type ?? throw new OriginException("Type is invalid.");
        ...
        {
            if(m_Type == null)
                throw new OriginException("Type is invalid.");

            string name = m_Type.FullName;
            return string.IsNullOrEmpty(m_Name) ? name : Utility.Text.Format("{0}.{1}" , name , m_Name);
        }
